...
int main(int argc, char *argv[]) {
	...
	if(process_rank == MASTER_RANK){
		...
		for(process_id = 1; process_id <= workers_num; process_id ++ ){
			...
			MPI_Send(&matrixA[offset][0], rows_num * N, MPI_INT, process_id, message_tag, MPI_COMM_WORLD);
			offset += rows_num;
		}
		...
		for(process_id = 1; process_id <= workers_num; process_id ++){
			...
			MPI_Recv(&resultMatrix[offset][0], rows_num * N, MPI_INT, process_id, message_tag, MPI_COMM_WORLD, &status);
		}
